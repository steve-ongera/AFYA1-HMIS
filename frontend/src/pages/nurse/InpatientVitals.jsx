...
export default function InpatientVitals() {
  ...
  const getVitalStatus = (value, type) => {
    // Simple validation - in production, use proper ranges
    if (!value) return ''
    const ranges = {
      temperature: { normal: [36.1, 37.2], warning: [37.3, 38.5], critical: [38.6, 42] },
      pulse_rate: { normal: [60, 100], warning: [40, 59], critical: [0, 39] },
              ...
              <div className="card-body">
                <div className="table-wrapper">
                  <table className="table">
                    <thead>
                      <tr><th>Patient</th><th>Admission #</th><th>Bed</th><th>Doctor</th><th>Last Vitals</th><th>Actions</th></tr>
                    </thead>
                    <tbody>
                      {admissions.map((admission) => (
                        <tr key={admission.id}>
                          ...
                          <td>{admission.vital_signs?.[0]?.recorded_at ? new Date(admission.vital_signs[0].recorded_at).toLocaleString() : 'Not recorded'}</td>
                          <td>
            ...
            <div className="card" style={{ marginBottom: 24 }}>
              ...
              <div className="card-body">
                <div className="vitals-grid">
                  ...
                  <div className="vital-item">
                    ...
                    <input type="number" className="form-input" value={formData.blood_pressure_systolic} onChange={(e) => setFormData(prev => ({ ...prev, blood_pressure_systolic: e.target.value }))} />
                  </div>
                  <div className="vital-item">
                    <div className="vital-label">BP Diastolic</div>
                    <input type="number" className="form-input" value={formData.blood_pressure_diastolic} onChange={(e) => setFormData(prev => ({ ...prev, blood_pressure_diastolic: e.target.value }))} />
                  </div>
                  <div className="vital-item">
                    <div className="vital-label">Pulse Rate</div>
                    <input type="number" className="form-input" value={formData.pulse_rate} onChange={(e) => setFormData(prev => ({ ...prev, pulse_rate: e.target.value }))} />
                  </div>
                  <div className="vital-item">
                    <div className="vital-label">Respiratory Rate</div>
                    <input type="number" className="form-input" value={formData.respiratory_rate} onChange={(e) => setFormData(prev => ({ ...prev, respiratory_rate: e.target.value }))} />
                  </div>
                  <div className="vital-item">
                    <div className="vital-label">O2 Saturation (%)</div>
                    <input type="number" className="form-input" value={formData.oxygen_saturation} onChange={(e) => setFormData(prev => ({ ...prev, oxygen_saturation: e.target.value }))} />
                  </div>
                  <div className="vital-item">
                    <div className="vital-label">Weight (kg)</div>
                    <input type="number" step="0.1" className="form-input" value={formData.weight} onChange={(e) => setFormData(prev => ({ ...prev, weight: e.target.value }))} />
                  </div>
                  <div className="vital-item">
                    <div className="vital-label">Pain Score (0-10)</div>
                    <input type="number" min="0" max="10" className="form-input" value={formData.pain_score} onChange={(e) => setFormData(prev => ({ ...prev, pain_score: e.target.value }))} />
                  </div>
                </div>
              ...
              <div className="card-footer" style={{ display: 'flex', justifyContent: 'flex-end', gap: 12 }}>
                <button type="submit" className="btn btn-primary" disabled={recording}>
                  {recording ? <><span className="spinner" style={{ width: 16, height: 16 }}></span> Recording...</> : 'Record Vitals'}
                </button>
              </div>
            </div>
          </form>
                    ...
                    <tbody>
                      {vitalsHistory.map((vitals) => (
                        <tr key={vitals.id}>
                          ...
                          <td><span className={`vital-value ${getVitalStatus(parseFloat(vitals.temperature), 'temperature')}`}>{vitals.temperature || '-'}°C</span></td>
                          ...
                          <td><span className={`vital-value ${getVitalStatus(vitals.pulse_rate, 'pulse_rate')}`}>{vitals.pulse_rate || '-'}</span></td>
                          <td>{vitals.respiratory_rate || '-'}</td>
                          <td><span className={`vital-value ${getVitalStatus(vitals.oxygen_saturation, 'oxygen_saturation')}`}>{vitals.oxygen_saturation || '-'}%</span></td>
